...
预计单次发货量（必须引用 items 中的实际 predicted_weight 数值范围，如"预计单次发货量在 XX-XX 吨之间"）
预测置信度
主要原因
最终结论

⚠️ 重要约束：第六部分中的"预计单次发货量"、"预计发货次数"、"预计发货时间"必须与你输出的 JSON items 中的数据完全一致。不允许出现分析文本说"30-48吨"但 JSON 中 predicted_weight 实际为 90-140 吨的情况。必须先确定 JSON 中的 predicted_weight 数值，再在分析文本中引用这些真实数值。
