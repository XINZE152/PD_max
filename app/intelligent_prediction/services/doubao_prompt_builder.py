...
【第六部分：综合预测结论】
整体发货概率：高 / 中 / 低
预计发货次数（与 items 中 predicted_weight > 0 的条数一致）
预计发货时间（列出 items 中 predicted_weight > 0 的具体日期）
预计日发货量（⚠️ 必须按日期汇总：同一天可能有多条 items，日发货量 = 该日所有 items 的 predicted_weight 之和。引用范围时必须用日汇总值，如"预计日发货量在 XX-XX 吨之间"）
预测置信度
主要原因
最终结论
...
⚠️ 重要约束：
1. 第六部分中的"预计日发货量"必须基于按日期汇总后的数值，而不是单条 predicted_weight。例如：如果 6月3日 有 电动=80吨 和 电轿=50吨，则日发货量=130吨。
2. "预计发货次数"、"预计发货时间"、"预计日发货量"必须与你输出的 JSON items 中的数据完全一致。不允许出现分析文本说"30-48吨"但 JSON 中 predicted_weight 实际为 90-140 吨的情况。如果文本中预测了8次发货此时，则JSON中必须有8条 predicted_weight > 0 的记录，并且这些记录的日期必须与分析文本中提到的"预计发货时间"完全一致。
