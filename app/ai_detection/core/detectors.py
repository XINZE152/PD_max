...
class PixelLevelDetector:
    ...
    def _check_dct_anomaly(self, gray: np.ndarray) -> float:
        """8x8 分块 DCT 频域分析：检测 AI 生成/修补图像的高频异常。CPU 友好。"""
        try:
            h, w = gray.shape
            if h < 8 or w < 8:
                return 0.0
            ...
            for y in range(0, h - 7, 8):
                for x in range(0, w - 7, 8):
                    ...
                    hf = np.sum(np.abs(dct_block[4:, 4:]))
                    total = np.sum(np.abs(dct_block)) + 1e-6
                    dct_high_freq_ratios.append(hf / total)
            if not dct_high_freq_ratios:
                return 0.0
            mean_hf = np.mean(dct_high_freq_ratios)
            if mean_hf > 0.015:
