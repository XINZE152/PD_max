...
- 传入：选中仓库id列表、冶炼厂id列表、品类id列表、price_type（取价口径）、**吨数**（可选，默认 `1`；**展示与利润用的报价、运费均为元/吨，且报价为不含税**）
- 输出：
  - `data`：明细列表（含 `仓库id/冶炼厂id/品类id`、`吨数`、**`利润`**=**不含税**报价×吨数−运费×吨数；若 `price_type` 为含 1%/3%/13% 增值税，接口会先将含税价除以 (1+税率) 得到不含税价再写入 `报价` 并算利润；无报价时 `报价` 为 `null`，`报价来源` 为 `unavailable`）
...
```json
{
  "code": 200,
  "data": [
    { "仓库id": 101, "冶炼厂id": 201, "品类id": 301, "仓库": "北京仓", "冶炼厂": "华北冶炼厂", "品类": "铜", "price_type": "3%增值税", "吨数": 10.5, "运费": 200, "报价": 9453.4, "报价来源": "direct", "利润": 97160.7 },
    { "仓库id": 101, "冶炼厂id": 201, "品类id": 302, "仓库": "北京仓", "冶炼厂": "华北冶炼厂", "品类": "铝", "price_type": "3%增值税", "吨数": 10.5, "运费": 200, "报价": 8106.8, "报价来源": "calc_from_base", "利润": 83021.4 },
    { "仓库id": 102, "冶炼厂id": 201, "品类id": 301, "仓库": "上海仓", "冶炼厂": "华北冶炼厂", "品类": "铜", "price_type": "3%增值税", "吨数": 10.5, "运费": 300, "报价": null, "报价来源": "unavailable", "利润": -3150.0 }
  ],
  "冶炼厂利润排行": [
...
- 逻辑说明：
  1. 读取全部启用冶炼厂 ID；查询各(仓库, 冶炼厂)最新运费、各(冶炼厂, 品类)最新报价
  2. 计算综合成本（报价 + 运费），按「每条需求 × 每个冶炼厂」展开为结构化数据
  3. 将数据交由大语言模型分析，要求：同仓库货物可混装、尽量整车（20–30 吨）、优先低成本方案
  4. 返回 LLM 文本 `suggestion` 与明细 `raw`（`raw` 中每条对应一行：某冶炼厂、某品类、某需求吨数、在某仓库下的运费与综合成本）
- 模拟请求JSON：
```json
{
