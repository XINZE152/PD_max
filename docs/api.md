...
- 传入：选中仓库id列表、冶炼厂id列表、品类id列表、price_type（取价口径）、**吨数**（可选，默认 `1`；未传 **品类吨数列表** 时所有品类共用）、**品类吨数列表**（可选；多品类不同重量混选时使用：`[{"品类id":301,"吨数":5},{"品类id":302,"吨数":12}]`，须与 **品类id列表** 中每个 id **恰好一条**，覆盖后各行按对应品类吨数计算总价/运费/利润）、**最优价计税口径列表**（可选，默认 `["3pct"]`）、**最优价排序口径**（可选，须在列表中；省略则用列表第一项，决定明细排序及 **`总回收价`** 所用单价档）、**报价日期**（可选 `YYYY-MM-DD`；传入则只取该日的 `quote_details`，不传则以比价基准日（默认 `Asia/Shanghai` 当天，环境变量 `QUOTE_COMPARISON_TZ`）为参照，每个冶炼厂+品种在 `quote_details` 中取 **与该日日历距离最近** 的 `quote_date` 一条；若多条距离相同，则取 **`created_at` 最新** 的一条）。**明细范围**：对每个「选中仓库 × 启用中的选中冶炼厂 × 选中品类」均返回一行；`冶炼厂id列表` 去掉停用厂后 **顺序与请求一致**（去重）；若某仓库—冶炼厂尚无 `freight_rates` 记录，**运费单价按 0** 仍返回该行。计价关系：**总价** = **单价**×**该品类吨数**（与 **`报价金额`** 一致，**`单价`/`报价`** 为按 `price_type` 折合后的元/吨不含税展示价，**与最优价排序口径无关**）；**总回收价** = **总回收价元每吨**×**该品类吨数**（冶炼厂按 **最优价排序口径** 支付的整单货款，**不减运费**）；**运费** = **运费单价**×**该品类吨数**（与 **`总运费`** 一致）；**利润** = **总价 − 运费**（主行展示利润，与 `price_type` 一致）；**`最优价各口径利润[b]`** = 该口径元/吨单价×吨数 − **总运费**。有报价时：**`总回收价` = `最优价各口径利润[最优价排序口径]` + `总运费`**。明细与冶炼厂排行按 **最优价排序口径** 对应利润排序；**`冶炼厂利润排行`** 汇总各口径利润合计。
- 输出：
  - `data`：明细列表含 **`单价`**、**`总价`**、**`总回收价元每吨`**、**`总回收价`**、**`运费单价`**、**`运费`**、**`总运费`**、**`报价`**、**`报价金额`**、`运费计价方式`（固定 `per_ton`）、`利润`、`基准价`、`含3%税价`、`最优价各口径利润` 等；响应顶层 **`最优价排序口径`** 标明本次 **`总回收价`** 所用档位。若 `price_type` 为含 1%/3%/13% 增值税，先将含税价折合为不含税再写入 `单价`/`报价`/`总价`；**`总回收价`** 仍按 **最优价排序口径** 取对应含税/不含税单价（与 `最优价各口径利润` 同源）。无报价时 `单价`/`报价`/`总价`/`报价金额`/ **`总回收价`**/ **`总回收价元每吨`** 为 `null`，`报价来源` 为 `unavailable`，此时 **`利润`** = **−`总运费`**
  - **`冶炼厂利润排行`**：按冶炼厂汇总，**从高到低** 按所选最优价排序口径下的利润合计排序
  - **循融宝**：启用循融宝发货的冶炼厂，明细顶层仍为含循融宝口径；**`不含循融宝`** / **`含循融宝`** 子对象内同样含 **`总回收价`**、**`总回收价元每吨`**（分别按不加价/加价报价行计算）
  - **前端提示**：到库利润请使用 **`利润`** 或 **`最优价各口径利润[最优价排序口径]`**；冶炼厂应付货款（不含运费扣减）用 **`总回收价`**；**`运费单价`** 为元/吨，**`总运费`** 为全程运费（元），勿与 **`总回收价`** 混淆。

**总回收价与排序口径（`最优价排序口径`）**

| 字段 | 含义 |
|------|------|
| `总回收价元每吨` | 由请求/响应中的 **`最优价排序口径`** 决定档位单价（元/吨），与 `最优价各口径利润` 各档单价来源相同（`base`/`1pct`/`3pct`/`13pct`/`normal_invoice`/`reverse_invoice`） |
| `总回收价` | `总回收价元每吨 × 吨数`（保留 2 位小数），**不含运费扣减** |
| `总价` / `报价金额` | 仍为 `price_type` 折合**不含税**展示价，切换最优价排序口径时**不变** |

口径对应示例（同一报价行、27 吨、河南金利基准不含税 9706 元/吨时）：

- **`最优价排序口径`: `"base"`** → `总回收价元每吨` = `9706`，`总回收价` = `262062`（= `9706 × 27`）；通常与 `基准价` 一致
- **`最优价排序口径`: `"3pct"`** → `总回收价元每吨` = `含3%税价`，`总回收价` 高于 base；且 **`总回收价` = `利润_含3%` + `总运费`**

请求示例（按基准口径排序并返回 base/3pct 两档利润）：
...
将 **`最优价排序口径`** 改为 `"3pct"` 后，同一明细行 **`总价`** 不变，**`总回收价`** 升高为含 3% 税整单货款。
...
- 传入：`full_data`（可选，VLM/Excel 全量数据原样回传）、报价明细列表（前端确认/修改后；**每条须含报价日期**）
- 输出：写入状态
- **报价日期**：每条明细必填 `报价日期`（别名：`日期`、`quote_date`），格式 `YYYY-MM-DD`；无顶层日期字段。
- **`同冶炼厂当日整表覆盖`**：可选，默认 `false`；为 `true` 时按本次请求中出现的 **(冶炼厂, 报价日期)** 组合删除旧明细后再写入（整单 OCR/Excel 确认建议传 `true`）。
- **明细字段别名**（以下键名等价，服务端均会映射为规范键后落库）：

| 规范键 | 可接受的别名 |
|--------|-------------|
| `报价日期` | `日期`、`quote_date` |
| `冶炼厂名` | `冶炼厂`、`factory_name`、`name` |
| `冶炼厂id` | `factory_id`、`id` |
| `品类名` | `品类`、`category_name`、`品名`、`名称` |
| `品类id` | `category_id`、`品类Id` |

- 逻辑说明：
  1. 前端对接口5/OCR 或 Excel 上传返回的 `items` 确认/修正后，连同原始 `full_data` 一起提交
  2. `冶炼厂id` 为 null 时：按名称查 `dict_factories`，须与字典名称完全一致
  3. 品类处理：按 `品类名` 查 `dict_categories`，存在则复用其 `category_id`，不存在则自动新建
  4. `full_data` 存入 `quote_table_metadata`（按每条明细的 `factory_id + 行级 quote_date` 关联）
  5. 每条明细以 `(行级报价日期, 冶炼厂id, 品类名)` 为唯一键写入 `quote_details`
- 模拟请求JSON：
```json
{
  "full_data": {
