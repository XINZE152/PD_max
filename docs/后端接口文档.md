...
| DELETE | `/tl/purge_category` | 永久删除品类分组（硬删除；须先软删；默认级联） |
| DELETE | `/tl/purge_category_row` | 永久删除单条品类别名（硬删除；须先软删；默认级联） |
...
| GET | `/tl/warehouse_inventories` | 库房当前库存列表（每库房每品类最新一条） |
| POST | `/tl/warehouse_inventories` | 手工录入/覆盖库房某日按品类库存 |
| DELETE | `/tl/warehouse_inventories` | 删除库房某日库存快照（可选按品类） |
| POST | `/tl/import_warehouse_inventory_excel` | Excel 导入库房库存 |
| GET | `/tl/download_warehouse_inventory_template_excel` | 下载库房库存导入模板 |
| GET | `/tl/warehouse_receipt_prices` | 库房按品种收货价格列表（当前最新价） |
| POST | `/tl/warehouse_receipt_prices` | 新增/覆盖库房按品种收货价格（写入历史快照） |
| PUT | `/tl/warehouse_receipt_prices/{price_id}` | 修改库房按品种收货价格 |
| DELETE | `/tl/warehouse_receipt_prices/{price_id}` | 删除库房按品种收货价格 |
| GET | `/tl/warehouse_receipt_price_history` | 库房按品种收货价格历史列表 |
| DELETE | `/tl/warehouse_receipt_price_history/{history_id}` | 删除收货价格历史记录 |
| POST | `/tl/import_warehouse_receipt_prices_excel` | Excel 导入库房收货价格 |
| GET | `/tl/download_warehouse_receipt_prices_template_excel` | 下载库房收货价格导入模板 |
...
## 6. 垂直库房 AI 定价分析（独立模块，前缀 `/vertical-warehouse-ai`）

选定单个源库房，聚合其出边绑定与绑定库房间电动车电瓶收货价/距离/实时价差（收货价之差）/阶梯价差，调用大模型（`LLM_*` / `VLM_API_KEY` 兜底）生成三方面建议。竞品类型由 `VERTICAL_WAREHOUSE_AI_COMPETITOR_TYPE_*` 配置。无出边 → `400`「该库房没有绑定其他库房，无法预测」。

| 方法 | 路径 | 摘要 |
|------|------|------|
| GET | `/vertical-warehouse-ai/analysis` | 实时分析（Query：`warehouse_id`，可选 `as_of_date`） |
| POST | `/vertical-warehouse-ai/snapshots` | 生成快照 |
| GET | `/vertical-warehouse-ai/snapshots` | 快照列表 |
| GET | `/vertical-warehouse-ai/snapshots/{snapshot_id}` | 快照详情 |
| PUT | `/vertical-warehouse-ai/snapshots/{snapshot_id}` | 更新元数据 |
| DELETE | `/vertical-warehouse-ai/snapshots/{snapshot_id}` | 删除快照 |

---

## 7. 智能预测模块（`INTELLIGENT_PREDICTION_ENABLED` 开启时）
